...
`@agala-labs/ui` is Agala Labs' Vue 3 + TypeScript component library, built with Vite, scoped CSS, HSL design tokens, and accessible primitives. Vue is the peer dependency (`>=3.5.0 <4`); most components use `reka-ui` as headless behavior primitives underneath the Agala styling/API surface (`ColorPicker` is the deliberate exception — Reka's semantics don't match its confirm/rollback behavior). Current source also uses `date-fns` for date utilities and a private `@lucide/vue` registry behind `AgalaIcon`.
...
- Feedback/overlay: `AgalaAlert`, `AgalaBadge`, `AgalaDrawer`, `AgalaModal`, `AgalaModalProvider`, `AgalaToastProvider`, `AgalaTooltip`, `AgalaProgress`, `AgalaSkeleton`, `AgalaEmptyState`
...
- `AgalaInput`: `v-model`, `size`, `variant` `default|ghost`, `error`, `errorMessage`, `disabled`, `readonly`, `iconStart`, `iconEnd`, `iconEndActionable`, `type`, `placeholder`, `id` (or legacy `inputId`), `name`, `required`, `autocomplete`, `ariaLabel`, `ariaLabelledby`, `ariaDescribedby`, `ariaDetails`, `ariaErrorMessage`, `ariaInvalid`, `ariaRequired`, `class`, `wrapperClass`; password inputs use an eye toggle. `class` targets the native `<input>` and `wrapperClass` targets the outer wrapper. Form and accessible naming props bind to the native `<input>`; pair `id`/`inputId` with `AgalaFormField` `htmlFor` to activate the label.
- `AgalaFormField`: `label`, `helper`, `error`, `disabled`, `htmlFor`, `required`, `class`; default slot for the control.
- `AgalaSelect`: `options`, `v-model`, `multiple`, `searchable`, `clearable`, `loading`, `onSearch`, `maxDisplayed`, `maxSelections`, `id` (or legacy `inputId`), `name`, `required`, `autocomplete`, `ariaLabel`, `ariaLabelledby`, `ariaDescribedby`, `ariaDetails`, `ariaErrorMessage`, `ariaInvalid`, `ariaRequired`; uses per-instance collision-aware listbox IDs and supports grouped/subtitled disabled options. Its option menu is never narrower than the trigger, expands narrow triggers up to a legible `14rem` target, and remains capped by the available viewport. Form attributes are represented by the Reka native select/input when `name` is set.
- `AgalaCreatableSelect`: multi-select chips with `options`, `v-model`, `creatable`, `labelKey`, `idKey`, `maxDisplayed`, `debounce`, `id` (or legacy `inputId`), `name`, `required`, `autocomplete`, and the same ARIA naming/description props; emits `create` and `search`. A native hidden form input is used for `name`.
- `AgalaDatePicker`: `v-model`, `size`, `min`, `max`, `clearable`, `inline`, `highlightDates`, `displayMonth`, `id` (or legacy `inputId`), `name`, `required`, `autocomplete`, and the same ARIA naming/description props; emits `update:displayMonth`. A native date input is used for `name`.
...
- `AgalaTabs`: content-panel navigation with `tabs`, `v-model`, `variant underline|pills`, and `orientation horizontal|vertical`, `ariaLabel`; each `TabItem` supports `value`, `label`, `disabled`, and optional explicit `tabId`/`panelId` IDs. Panel slots are named `panel-<value>` and custom labels use `tab-<value>` with `{ tab, active }`. Internal panel slots keep generated IDs by default; when composing panels outside Tabs, pass `panelId` and render the matching external `role=tabpanel` with `aria-labelledby` pointing to `tabId`. `aria-controls` is omitted when neither an internal panel slot nor `panelId` exists. Underline is the canonical primary treatment. Pills are separated soft navigation items without an enclosing track or raised tile. Vertical orientation renders a genuine side rail beside one adjacent panel that reads top-to-bottom and uses Up/Down plus Home/End; horizontal uses Left/Right plus Home/End; disabled tabs are skipped. Custom labels must not contain interactive controls. Do not use Tabs for a form value or immediate view-mode setting.
...
- `AgalaListGroup`: `variant divided|cards`, `gap`, `borderless`, `dividers`; `AgalaListGroupItem` has label/subtitle/icon/badge/actionIcon/radius, semantic `badgeVariant default|primary|success|warning|danger`, and `leading`, default, `trailing`, `badge` slots. Items are non-actionable `div[role=listitem]` by default; use `interactive` for a native button or `as="button"|"a"` with `href` for actionable semantics. Disabled links expose `aria-disabled` and prevent navigation. The badge and trailing slots override the built-in badge treatment.
- `AgalaTextarea`: `v-model`, `rows`, `resize`, `disabled`, `readonly`, `error`, `errorMessage`, `placeholder`, `id`, `name`, `required`, `autocomplete`, and `aria*` attributes. Standard `aria-*` and `data-*` attributes are forwarded to the native textarea; pair `id` with `AgalaFormField html-for` for labels.
...
- `AgalaAlert`: application banner with a quiet neutral surface, semantic icon treatment, strong title, supporting body, and optional action. Props include `variant info|success|warning|danger`, `title`, `role alert|status|region` (default `status`), `dismissible`, `flat`, `icon?: string|false`, `dismissLabel`, and `ariaLabel`; slots are default body and `action`, and dismissal emits `dismiss`. Actions align centrally on wide layouts and wrap below the message under 639px. Use `role="alert"` only for urgent, dynamically introduced messages.
...
- Anchored overlays (`AgalaDropdownMenu`, `AgalaTooltip`, `AgalaSelect`, `AgalaCreatableSelect`, `AgalaModal`, `AgalaDrawer`, `AgalaToastProvider`, `AgalaAccordion`) are built on `reka-ui` primitives, teleported via Reka's own internal `<Teleport>`. Positioning, flip/shift-on-collision, focus containment, and outside-dismiss are Reka's, not a shared internal composable. They stack via `z-index` (Agala's layer scale), not the browser's native Popover API.
- Vue's scoped-CSS `data-v-*` attribute does not propagate through Reka's internal `<Teleport>`, so each of these components scopes its portaled-content CSS rules with `:global()` compounded on a static wrapper class unique to that component (e.g. `.agala-select-portal .dropdown`), rather than plain scoped selectors. Keep that pattern for any portaled content in these components; don't use a bare `:global(.someGenericClass)` — it can collide with unrelated classes elsewhere on a consuming page.
- `AgalaColorPicker` is the one exception: it still uses the pre-Reka pattern — `useFloatingOverlay` (`@floating-ui/vue` middleware: offset/flip/shift/size, automatic scroll/resize updates, 8px viewport margin) and `usePopoverBehavior` (native `popover="manual"`, centralized outside-click/scroll-close). It was deliberately not migrated to Reka's `ColorArea`/`ColorSlider`/`ColorField` — their emit-on-every-move and hex-canonicalization semantics don't match this component's confirm/rollback/clearable behavior. Do not reuse these two composables for new anchored overlays; they exist only to serve `ColorPicker`.
...
- At 320px and wider, components must not create document-level horizontal overflow. Tables and long tab strips may scroll inside their own bounds; anchored overlays must remain within an 8px viewport margin.
