...
case "$digest" in sha256:????????????????????????????????????????????????????????????????) ;; *) printf '%s\n' 'registry did not return an exact sha256 image digest' >&2; exit 1 ;; esac
case "${digest#sha256:}" in *[!0-9a-f]*) printf '%s\n' 'registry returned a non-hexadecimal image digest' >&2; exit 1 ;; esac

config=$("$crane_bin" config "$repository@$digest")
expected_label="\"org.opencontainers.image.revision\":\"$source_sha\""
case "$config" in *"$expected_label"*) ;; *) printf '%s\n' 'image revision label does not match the push commit' >&2; exit 1 ;; esac
