...
export const components: ComponentMeta[] = [
  ...
  },
  {
    ...
    props: [p('modelValue', 'string', 'Value used by v-model.'), p('size', "'sm' | 'md' | 'lg'", 'Control size.', "'md'"), p('variant', "'default' | 'ghost'", 'Visual treatment.', "'default'"), p('type', 'string', 'Native input type.', "'text'"), p('placeholder', 'string', 'Placeholder text.'), p('iconStart / iconEnd', 'string', 'Agala icon names.'), p('iconEndActionable', 'boolean', 'Makes the trailing icon interactive.', 'false'), p('error', 'boolean', 'Sets invalid styling.', 'false'), p('errorMessage', 'string', 'Inline error copy.'), p('disabled / readonly', 'boolean', 'Native input states.', 'false'), p('id / inputId', 'string', 'ID bound to the native input; inputId is the legacy alias.'), p('name', 'string', 'Native form field name.'), p('required', 'boolean', 'Native required state.', 'false'), p('autocomplete', 'string', 'Native autocomplete token.'), p('ariaLabel / ariaLabelledby', 'string', 'Accessible name or referenced label id.'), p('ariaDescribedby / ariaDetails / ariaErrorMessage', 'string', 'Accessible supporting and validation references.'), p('ariaInvalid / ariaRequired', 'boolean | string', 'Explicit ARIA validation state.'), p('class', 'string', 'Class applied to the native input.'), p('wrapperClass', 'string', 'Class applied to the outer input wrapper.')],
    events: ['update:modelValue(value: string)', 'icon-end-click'], accessibility: 'For a visible label, pair with FormField. Form and ARIA attributes bind to the native input, never the wrapper; inputId remains a backwards-compatible alias for id.',
  ...
  },
  {
    ...
    props: [p('options', 'SelectOption[]', 'Required option collection.'), p('modelValue', 'string | string[]', 'Selected value(s).'), p('multiple', 'boolean', 'Enables multi-select.', 'false'), p('searchable', 'boolean', 'Shows filtering input.', 'false'), p('clearable', 'boolean', 'Allows clearing the selection.', 'false'), p('loading', 'boolean', 'Shows a loading state.', 'false'), p('size', "'sm' | 'md' | 'lg'", 'Control size.', "'md'"), p('placeholder', 'string', 'Empty selection copy.'), p('maxDisplayed', 'number', 'Maximum visible chips.'), p('maxSelections', 'number', 'Selection limit.'), p('error / errorMessage', 'boolean / string', 'Validation state and copy.'), p('id / inputId', 'string', 'ID bound to the combobox trigger.'), p('name', 'string', 'Native form field name.'), p('required', 'boolean', 'Required state.', 'false'), p('autocomplete', 'string', 'Native form autocomplete token.'), p('ariaLabel / ariaLabelledby', 'string', 'Accessible name or referenced label id.'), p('ariaDescribedby / ariaDetails / ariaErrorMessage', 'string', 'Accessible supporting and validation references.')],
    events: ['update:modelValue(value)', 'search(query: string)'], accessibility: 'Implements combobox/listbox semantics with keyboard navigation, selected state, and expanded state. The option menu is never narrower than its trigger, expands narrow triggers to a legible width, and remains capped by the available viewport.',
  ...
  },
  {
    ...
    props: [p('options', 'CreatableSelectOption[]', 'Required options.'), p('modelValue', 'string[]', 'Selected IDs.'), p('creatable', 'boolean', 'Allows new values.', 'true'), p('labelKey / idKey', 'string', 'Keys used for custom records.'), p('maxDisplayed', 'number', 'Maximum visible chips.'), p('debounce', 'number', 'Search debounce in milliseconds.'), p('disabled', 'boolean', 'Disables interaction.', 'false'), p('id / inputId', 'string', 'ID bound to the combobox trigger.'), p('name', 'string', 'Native form field name.'), p('required', 'boolean', 'Required state.', 'false'), p('autocomplete', 'string', 'Native form autocomplete token.'), p('ariaLabel / ariaLabelledby', 'string', 'Accessible name or referenced label id.'), p('ariaDescribedby / ariaDetails / ariaErrorMessage', 'string', 'Accessible supporting and validation references.')],
  ...
  {
    slug: 'section-nav', name: 'Section Nav', exports: ['AgalaSectionNav'], description: 'Navigates peer sections within one settings or detail surface.',
    props: [p('items', 'SectionNavItem[]', 'One-level destinations with value, label, and optional icon, href, disabled state, or badge.'), p('modelValue', 'string', 'Required active item value.'), p('variant', "'panel' | 'plain'", 'Surface treatment.', "'panel'"), p('density', "'compact' | 'comfortable'", 'Row density.', "'compact'"), p('responsive', "'scroll' | 'stack'", 'Narrow-screen behavior.', "'scroll'"), p('ariaLabel', 'string', 'Accessible navigation name.', "'Section navigation'"), p('class', 'string', 'Consumer override class.')],
    events: ['update:modelValue(value: string)', 'select(item, event)'], accessibility: 'Uses native navigation, links, and buttons. Active links expose aria-current="page"; disabled destinations remain visible and unavailable. Scroll mode reveals the active item without widening the document.',
    snippet: `<AgalaSectionNav v-model="section" :items="sections" aria-label="Configuration sections" />`,
  },
  {
